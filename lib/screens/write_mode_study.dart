...
class _WriteModeStudyState extends ConsumerState<WriteModeStudy> {
  ...
  Flashcards? _selectNextFlashcard(Flashcards flashcard) {
    if (initialBox.isNotEmpty) return initialBox[Random().nextInt(initialBox.length)];
    if (wrongBox.isNotEmpty) return _getRandomFromBox(wrongBox, flashcard);
    if (firstRightBox.isNotEmpty) return _getRandomFromBox(firstRightBox, flashcard);
    return null;
  }

  Flashcards? _getRandomFromBox(List<Flashcards> box, Flashcards currentCard) {
    if (box.length == 1 && box.first == currentCard) {
      return firstRightBox.isEmpty ? box.first : firstRightBox[Random().nextInt(firstRightBox.length)];
    }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<List<Flashcards>>>(flashcardStreamProvider(widget.setId!), (previous, next) {
      if (next.hasValue && next.value!.isNotEmpty) {
        setState(() {
          initialBox = List<Flashcards>.from(next.value!);
          ref.read(displayedFlashcardProvider.notifier).state = initialBox[Random().nextInt(initialBox.length)];
        });
      }
    });
    ...
    return Scaffold(
      ...
      body: flashcardsAsync.when(
        data: (flashcards) => flashcards.isEmpty ? const EmptyContainer(emptyType: EmptyType.card) : _buildCardDisplay(context, colors),
  ...
  Widget _buildCardDisplay(BuildContext context, ColorScheme colors) {
    ...
    if (selectedFlashcard == null) return const Center(child: CircularProgressIndicator());
  ...
  Container _buildCardContainer(Flashcards flashcard, ColorScheme colors) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.25), offset: const Offset(2, 2), blurRadius: 6)],
      ),
  ...
  Column _buildCardContent(Flashcards flashcard, ColorScheme colors) {
    return Column(
      ...
      children: [
        const DisplayText(text: "Câu hỏi / định nghĩa", color: Colors.black, fontSize: 15),
        const Gap(5),
        DisplayText(text: flashcard.frontContent, color: Colors.black, fontSize: 19),
        const Gap(20),
        const DisplayText(text: "Câu trả lời", color: Colors.black, fontSize: 15),
        const Gap(10),
  ...
  TextFormField _buildAnswerTextField() {
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: const BorderSide(width: 1, color: Colors.white)),
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: BorderSide(width: 1, color: context.colorScheme.primary)),
      ),
      ...
      children: [
        ElevatedButton(
          onPressed: () {
            setState(() {
              isAnswered = true;
              isCorrect = answerController.text.toLowerCase().trim() == flashcard.backContent.toLowerCase().trim();
              newFlashcard = _getNextFlashcard(isCorrect, flashcard);
            });
          },
          style: ElevatedButton.styleFrom(backgroundColor: colors.primary),
          child: const Padding(padding: EdgeInsets.symmetric(vertical: 15, horizontal: 5), child: DisplayText(text: "Trả lời")),
        ),
        const Gap(10),
  ...
  ElevatedButton _buildNextCardButton(ColorScheme colors) {
    return ElevatedButton(
      onPressed: !isAnswered
          ...
          : () {
              if (newFlashcard != null) {
                ref.read(displayedFlashcardProvider.notifier).state = newFlashcard;
      ...
      style: ElevatedButton.styleFrom(backgroundColor: colors.primary),
      child: const Padding(padding: EdgeInsets.symmetric(vertical: 15), child: Icon(Icons.arrow_right_alt, color: Colors.white)),
    );
  }
